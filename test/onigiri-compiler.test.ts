  ...
  describe('snapshots', () => {
    ...
    describe('dynamic bindings', () => {
      ...
      it('class object binding', () => {
        const result = compileOnigiri(`<div :class="{ active: isActive, disabled: isDisabled }">Content</div>`)
        expect(result.code).toMatchSnapshot()
      })
      ...
      it('style object binding', () => {
        const result = compileOnigiri(`<div :style="{ color: textColor, fontSize: size + 'px' }">Content</div>`)
        expect(result.code).toMatchSnapshot()
      })
    })
    ...
    describe('directives', () => {
      ...
      it('v-for with key', () => {
        const result = compileOnigiri(`<li v-for="item in items" :key="item.id">{{ item.name }}</li>`)
        expect(result.code).toMatchSnapshot()
      })

      it('v-for with index', () => {
        const result = compileOnigiri(`<li v-for="(item, index) in items" :key="index">{{ index }}: {{ item }}</li>`)
        expect(result.code).toMatchSnapshot()
      })
