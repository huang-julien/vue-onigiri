...
export function prefixIdentifiers(
  ...
  try {
    ...
    try {
      ...
      ast = parse(content, { plugins: ['typescript'], sourceType: 'module', allowReturnOutsideFunction: true }).program
    }
    catch {
 ...
 */
function wrapEventHandler(content: string, kind: 'member' | 'fn' | 'statement' | undefined, context: CodegenContext): string {
  const trimmed = content.trim()
...
export function genExpressionAsValue(node: ExpressionNode | undefined, context: CodegenContext): void {
  if (!node) {
...
export function genEventHandler(node: ExpressionNode | undefined, context: CodegenContext): void {
  ...
  if (node.type === NodeTypes.SIMPLE_EXPRESSION) {
    const simpleNode = node as SimpleExpressionNode & { _onigiriEventKind?: 'member' | 'fn' | 'statement' }
    if (simpleNode.isStatic) {
      context.push(simpleNode.content)
    }
    else {
